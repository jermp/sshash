...
//             m_num_bits_per_relative_offset = m_nb.per_relative_offset;
//             std::vector<uint64_t>().swap(m_v);
//         }
...
//         uint64_t relative_offset = encoded_offset & ((1ULL << m_num_bits_per_relative_offset) -
//         1); uint64_t string_id = encoded_offset >> m_num_bits_per_relative_offset;
...
//         uint64_t contig_end = q.string_end;
...
//     // std::pair<uint64_t, uint64_t> next_geq(uint64_t lower_bound) const {
//     //     auto [pos, val] = m_seq.next_geq(lower_bound);
//     //     return {pos, val};
//     // }
